...
本文档旨在详述 Naive RAG (对照组) 与 Advanced RAG (实验组) 双盲无偏量化评测体系的架构与实现流程。本体系采用分段离线流水线（Segmented Offline Pipeline）架构设计，防止大模型在本地显卡上并发运行时导致显存溢出 (OOM)，并由评测控制中心主脚本进行统一调度。
...
> ⚠️ **数据底座对齐前提**：测试正式开始前，Naive RAG 数据库需同步导入目标知识库文件 `E:/desktop/code/New folder/paper song.docx`，而 Advanced RAG 数据库保留该论文及已有的 40 多篇日常开发/调试日志（作为干扰背景噪音），以模拟真实的“大海捞针”检索场景。
...
* **出题官 / 打分裁判 (Judge LLM)**：`qwen3.6-35b-a3b-distilled-think` (搭载 Claude 4.7 蒸馏思考链，保证题目和评分标准的高可信度，温度 `0.0` 确保打分一致性)。
...
├── evaluation_set_generator.py # 阶段 1：无偏中立自动出题
├── run_retrieval.py         # 阶段 2：数据对齐与双轨检索提取
...
* **原理**：直接读取 `paper song.docx` 物理文件，按固定 1000 字符中立切分（无重叠），随机选择 30 个片段输入 Qwen-Think，生成 30 组 `(Question, Ground Truth)` 存入 `tests/test_dataset.json`。
* **Prompt 模板**：
  ```text
  你是一个严谨的学术评测出题官。请阅读以下从论文中提取的文本片段，为其设计一个具体的技术性问题，并给出该问题在原文中能够直接印证的标准答案（Ground Truth）。
  
  【限制要求】：
  1. 问题必须针对文本中的核心技术细节、公式或实验结论，切忌泛泛而谈。
  2. 标准答案必须完全忠实于原文，不得夹杂任何外部知识。
  3. 请严格按照以下 JSON 格式输出，不要包含任何解释或 markdown 标记：
  {
    "question": "问题内容",
    "ground_truth": "标准答案内容"
  }
...
* **原理**：
  1. 动态引入 `E:/project/rag/rag_engine.py`，检查 Naive 向量库中是否已索引 `paper song.docx`，若无则自动触发索引对齐。
  2. 载入 `test_dataset.json`，遍历问题，分别调用 Naive RAG 的 `search_with_context` 和 Advanced RAG 的 `RAGCoordinator.query`，提取上下文。
  3. 写入 `tests/retrieval_results.json`，包含 `naive_context` 与 `advanced_context`。
...
主脚本 `tests/run_pipeline.py` 将提供极简的 CLI 菜单交互：
